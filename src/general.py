...
from .pred_ign import PredIGNManager
...
class ManagerType(Enum):
    ...
    def get_displayable_name(type: str) -> str:
        ...
        if type == ManagerType.PRED_IGN.value: return "Habitat map by aerial imagery"
    ...
    def get_description(type: str) -> str:
        ...
        if type == ManagerType.PRED_IGN.value: return "Habitat map by aerial imagery using a multilabel semantic segmentation model."
...
class GeneralManager:
    ...
    def __init__(self, data_path: Path) -> None:
        ...
        self.pred_ign_manager = PredIGNManager(Path(data_path, ManagerType.PRED_IGN.value))
    ...
    def get_tile(self, collection_type: str, year: str, params: ParametersCOG) -> ImageData | None:
        ...
        elif collection_type == ManagerType.PRED_IGN.value:
            tile = self.pred_ign_manager.get_tile(year, params)
    ...
    def get_prediction_drone(self, lon: float, lat: float, year: str) -> str | None:
        return self.pred_drone_manager.get_prediction(lon, lat, year)
    

    def get_prediction_ign(self, lon: float, lat: float, year: str) -> str | None:
        return self.pred_ign_manager.get_prediction(lon, lat, year)
